...
# ------------------------------------------------------------
# 2. FCN 网络：支持 FCN-32s, FCN-16s, FCN-8s
# ------------------------------------------------------------
class FCN(nn.Module):
    """
    ...
    def forward(self, x):
        pool3, pool4, conv7 = self.backbone(x) # torch.Size([1, 256, 28, 28]) torch.Size([1, 512, 14, 14]) torch.Size([1, 4096, 7, 7])
...
# ------------------------------------------------------------
# 3. main 函数：测试 FCN-32s, FCN-16s, FCN-8s 的前向传播
# ------------------------------------------------------------
def main():
    # 参数设置
    ...
    for version in ['32s', '16s', '8s']:
        print(f"\n--- Testing FCN-{version} ---")
        model = FCN(num_classes=num_classes, version=version)
        model.eval()  # 测试模式
